        ...
        GeometryReader { geometry in
            ZStack {
                ...
                if coordinator.startPhase == .countdown, let countdownStartedAt = coordinator.countdownStartedAt {
                    ...
                    BallrDrillReadinessOverlay(ballFoundStartedAt: coordinator.handFoundStartedAt)
                }
            }
...
private final class HandTargetCoordinator: ObservableObject {
    ...
    private let requiredHandLockSeconds: TimeInterval = 3.0
    ...
    func prepare(in size: CGSize, forceRespawn: Bool = false) {
        ...
        gameState.prepare(in: self.size, forceRespawn: forceRespawn, allowSpawn: startPhase == .live)
        renderView?.update(
            hands: lastDetectedHands,
            isTracking: isTracking,
            target: gameState.target,
...
private final class HandTargetRenderView: UIView {
    ...
    private var handLayers: [Int: HandTargetHandLayerSet] = [:]
    ...
    private func renderHands() {
        ...
        for id in inactiveIDs {
            handLayers[id]?.container.removeFromSuperlayer()
            handLayers[id] = nil
        }

        for hand in hands {
            let layerSet = handLayers[hand.id] ?? makeHandLayerSet(for: hand)
            update(layerSet: layerSet, with: hand)
        }
    }

    private func makeHandLayerSet(for hand: HandTargetDetectedHand) -> HandTargetHandLayerSet {
        let container = CALayer()

        let fill = CAShapeLayer()
        fill.fillColor = UIColor.systemTeal.withAlphaComponent(0.16).cgColor
        ...
        let layerSet = HandTargetHandLayerSet(
            container: container,
            fill: fill,
            outline: outline,
            center: center,
        ...
        handLayers[hand.id] = layerSet
        return layerSet
    }

    private func update(layerSet: HandTargetHandLayerSet, with hand: HandTargetDetectedHand) {
        layerSet.container.isHidden = false
        layerSet.container.frame = hand.rect

        let localBounds = CGRect(origin: .zero, size: hand.rect.size)
        let path = UIBezierPath(roundedRect: localBounds, cornerRadius: 18).cgPath
        let tint = tintColor(for: hand.id)
        let opacity = max(0.2, min(hand.confidence, 1))

        layerSet.fill.frame = localBounds
        layerSet.fill.path = path
        layerSet.fill.fillColor = tint.withAlphaComponent(0.15 + opacity * 0.12).cgColor

        layerSet.outline.frame = localBounds
        layerSet.outline.path = path
        layerSet.outline.strokeColor = tint.withAlphaComponent(0.88).cgColor

        layerSet.center.frame = localBounds
        layerSet.center.path = UIBezierPath(
            ovalIn: CGRect(
                x: localBounds.midX - 6,
                y: localBounds.midY - 6,
                width: 12,
                height: 12
            )
        ).cgPath
        layerSet.center.fillColor = tint.cgColor

        layerSet.label.string = "HAND"
        layerSet.label.frame = CGRect(
            x: 0,
            y: max(6, localBounds.minY + 4),
            width: localBounds.width,
            height: 18
        )
    }

    private func tintColor(for handID: Int) -> UIColor {
        handID.isMultiple(of: 2)
            ? UIColor(red: 0.30, green: 0.92, blue: 0.85, alpha: 1)
            : UIColor(red: 0.95, green: 0.82, blue: 0.22, alpha: 1)
    }
    ...
    private func targetAgeProgress(for target: HandTargetTarget, at date: Date) -> Double {
        min(max(date.timeIntervalSince(target.spawnedAt) / HandTargetGameState.targetLifetime, 0), 1)
    }
...
private struct HandTargetHandLayerSet {
    let container: CALayer
    let fill: CAShapeLayer
    let outline: CAShapeLayer
    let center: CAShapeLayer
...
private struct HandTargetGameState {
    ...
    private let spawnTopFraction: CGFloat = 0.20
    ...
    mutating func step(
        ...
        if targetAge(for: currentTarget, at: timestamp) >= Self.targetLifetime {
            ...
            spawnTarget(in: size, timestamp: timestamp, previousCenter: currentTarget.center, avoiding: hands.map(\.rect))
            return .miss
        }
        ...
        if hands.contains(where: { handIntersectsTarget($0.rect, target: currentTarget) }) {
    ...
    private mutating func spawnTarget(
        ...
        for _ in 0..<72 {
            ...
            if isCandidateValid(
                ...
                target = HandTargetTarget(center: candidate, radius: radius, spawnedAt: timestamp)
                return
            }
        }

        target = HandTargetTarget(center: bestCandidate, radius: radius, spawnedAt: timestamp)
    }
    ...
    private func candidateQuality(
        ...
        if let farthestHandDistance = handRects.map({ distance(from: candidate, to: $0) }).min() {
    ...
    private func isCandidateValid(
        ...
        for handRect in handRects {
            let requiredClearance = targetRadius + max(baseClearance, min(handRect.width, handRect.height) * 0.2)
            if distance(from: candidate, to: handRect) < requiredClearance {
                return false
            }
        }
    ...
    private func handIntersectsTarget(_ handRect: CGRect, target: HandTargetTarget) -> Bool {
        distance(from: target.center, to: handRect) <= target.radius
    }
