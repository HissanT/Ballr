...
final class HandPoseCameraController: NSObject, ObservableObject {
    ...
    private func handCandidate(from observation: VNHumanHandPoseObservation) -> HandCandidate? {
        ...
        let minX = validPoints.map(\.location.x).min() ?? 0
        let maxX = validPoints.map(\.location.x).max() ?? 0
        let minY = validPoints.map(\.location.y).min() ?? 0
        let maxY = validPoints.map(\.location.y).max() ?? 0
        let averageConfidence = validPoints.reduce(0.0) { $0 + Double($1.confidence) } / Double(validPoints.count)

        let rawWidth = max(maxX - minX, 0.05)
        let rawHeight = max(maxY - minY, 0.06)
        let horizontalPadding = max(rawWidth * 0.22, 0.04)
        let lowerPadding = max(rawHeight * 0.22, 0.04)
        let upperPadding = max(rawHeight * 0.12, 0.025)

        let lowerLeftRect = CGRect(
            x: minX - horizontalPadding,
            y: minY - lowerPadding,
            width: rawWidth + horizontalPadding * 2,
            height: rawHeight + lowerPadding + upperPadding
        )

        let topLeftRect = CGRect(
            x: lowerLeftRect.minX,
            y: 1.0 - lowerLeftRect.maxY,
            width: lowerLeftRect.width,
            height: lowerLeftRect.height
        )

        let normalizedRect = expandedToMinimumSize(
            rect: topLeftRect.standardized.intersection(unitRect),
            minimumWidth: 0.12,
            minimumHeight: 0.14
        )
