...
def main() -> None:
    ...
    parser.add_argument(
        "--root",
        help="Review root. Defaults to training/dataset/review or training/dataset based on mode.",
    )
    parser.add_argument("--session", help="Only review one queue session folder")
    parser.add_argument(
        "--output-root",
        default=str(training_path("dataset_cleaned")),
    ...
    if args.mode == "queue":
        review_root = Path(args.root or training_path("dataset", "review"))
    ...
    if args.mode == "import-external":
        ...
        source_root = Path(args.root or training_path("dataset"))
