...
def _split_examples(records, root: Path, val_ratio: float) -> tuple[list[_Example], list[_Example]]:
    ...
    train_examples = [
        _Example(root / record.features_relpath, LABEL_TO_INDEX[record.label])
...
def main() -> None:
    ...
    class_weights = torch.tensor(
        [
            ...
            for index in range(len(EVENT_CLASS_ORDER))
        ],
        dtype=torch.float32,
        device=device,
    )
            ...
            torch.save(
                {
                    ...
                    "classes": [event_class.value for event_class in EVENT_CLASS_ORDER],
                    "hidden_size": args.hidden_size,
                    "num_layers": args.num_layers,
                    "dropout": args.dropout,
                },
