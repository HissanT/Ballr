...
def make_pose(
    ...
    *,
    ...
    if include_knees or kind == "knee" or kind == "thigh":
...
def run_sequence(state, frame_index: int, timestamp: float, previous_y: float, y_values: tuple[float, ...], *, kind: str, available: bool = True, include_knees: bool = True, include_hips: bool = True, ground_y: float = 420.0):
    event = None
    for y in y_values:
...
def test_juggle_mode_counts_thigh_contact():
    state, frame_index, timestamp, previous_y = warm_state()
    state, event, *_ = run_sequence(state, frame_index, timestamp, previous_y, THIGH_APEX, kind="thigh")

    assert event is not None
    assert event.event_class == JuggleEventClass.THIGH
    assert event.counted
...
def test_rearm_blocks_second_count_until_ball_rises_clear():
    ...
    state, second_event, *_ = run_sequence(
        ...
        timestamp,
        previous_y,
        (252.0, 258.0, 263.0, 267.0, 263.0, 258.0, 254.0),
        kind="foot",
    )
