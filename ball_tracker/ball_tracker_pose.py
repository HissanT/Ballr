...
class PoseFrame:
    ...
    hip_line_y: Optional[float] = None
    ...
    def has_reference_geometry(self) -> bool:
        return (
            ...
            or self.knee_line_y is not None
            or self.hip_line_y is not None
        )
...
def _build_pose_frame(
    ...
    *,
    ...
    del frame_size
    hip_line_y = _hip_line_y(state.keypoints)
...
def _contact_candidates(
    ...
    for name in ("left_ankle", "right_ankle", "left_knee", "right_knee", "left_wrist", "right_wrist"):
...
def _knee_line_y(keypoints: dict[str, np.ndarray]) -> Optional[float]:
    return _reference_line_y(keypoints, _KNEE_KEYS)


def _hip_line_y(keypoints: dict[str, np.ndarray]) -> Optional[float]:
    return _reference_line_y(keypoints, ("left_hip", "right_hip"))


def _reference_line_y(
    keypoints: dict[str, np.ndarray],
    names: tuple[str, ...],
) -> Optional[float]:
    values = [float(keypoints[name][1]) for name in names if name in keypoints]
    if not values:
        return None
    if len(values) == 1:
        return values[0]

    higher_point = min(values)
    lower_point = max(values)
    if lower_point - higher_point >= POSE_HIGH_KNEE_SWITCH_PIXELS:
        return higher_point
