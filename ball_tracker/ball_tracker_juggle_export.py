        ...
        while True:
            ...
            manifest.append(
                JuggleCandidateRecord(
                    ...
                    predicted_event_label="count" if event.counted else ("ground" if event.event_class.value == "ground" else "other"),
                    predicted_body_part_label=event.event_class.value if event.counted and event.event_class.value in {"foot", "knee", "hip"} else "",
                )
            )
            # Persist progress incrementally so interrupted export sessions remain reviewable.
