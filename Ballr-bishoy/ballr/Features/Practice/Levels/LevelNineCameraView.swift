...
private struct LevelNineGameState {
    ...
    private let xSimilarityMultiplier: CGFloat = 1.15
    ...
    mutating func step(
        ...
        if let currentHandTarget, hands.contains(where: { handIntersectsTarget($0, target: currentHandTarget) }) {
            handTarget = nil
        }

        if ballTarget == nil && handTarget == nil {
            hitStreak += 1
            let popupCenter = currentPairCenter ?? CGPoint(x: size.midX, y: size.midY)
            scorePopups.append(
                LevelNineScorePopup(
                    points: pairValue,
                    center: popupCenter,
            ...
            spawnPair(
                in: size,
                timestamp: timestamp,
                previousPairCenter: currentPairCenter,
                ballDisplayRect: ballDisplayRect,
                hands: hands
            )
            return .hit
    ...
    private mutating func spawnPair(
        ...
        for _ in 0..<96 {
            ...
            let handCandidate = randomTarget(
                xCenter: pairX,
                in: handBounds,
                radius: radius,
                xOffsetRange: radius * 0.45
            )
            let ballCandidate = randomTarget(
                xCenter: pairX,
                in: ballBounds,
                radius: radius,
                xOffsetRange: radius * 0.45
            ...
            guard xDifference <= radius * 1.1 else {
                continue
            }
    ...
    private func pairQuality(
        ...
        quality += abs(ballCandidate.x - handCandidate.x) * 1.25
    ...
    private func randomTarget(xCenter: CGFloat, in bounds: CGRect, radius: CGFloat, xOffsetRange: CGFloat) -> CGPoint {
        let x = min(max(xCenter + CGFloat.random(in: -xOffsetRange...xOffsetRange), bounds.minX), bounds.maxX)
