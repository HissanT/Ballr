        ...
        GeometryReader { geometry in
            ZStack {
                ...
                if cameraController.isStarting {
                    PracticeLevelLoadingOverlay(message: "Starting foot targets...")
                }
                ...
                if coordinator.startPhase == .readiness && cameraController.errorMessage == nil {
                    PracticeTrackedTargetReadinessOverlay(
                        lockStartedAt: coordinator.footFoundStartedAt,
                        requiredLockSeconds: 3.0,
                        searchingTitle: "Find your foot",
                        lockedTitle: "Hold your foot still",
                        searchingSubtitle: "Put your foot in frame to start."
                    )
                }
...
private final class FootTargetCoordinator: ObservableObject {
    ...
    func handle(frame: FootPoseFrame, cameraController: FootPoseCameraController) {
        ...
        if startPhase == .live, !isCompleted {
            ...
            if event == .hit {
                PracticeTargetScoreSoundPlayer.playScore(failureContext: "Foot target score")
                successfulHits += 1
