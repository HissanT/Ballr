...
final class BallTrackerCameraController: NSObject, ObservableObject {
    ...
    private let temporalFrameBuffer = TemporalGrayFrameBuffer(maxFrameAge: 0.35)
...
extension BallTrackerCameraController: AVCaptureVideoDataOutputSampleBufferDelegate {
    func captureOutput(
        ...
        if resources.config.inputTemporalMode == .temporalGray3 {
            guard
                let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer),
                let temporalPixelBuffer = temporalFrameBuffer.makeInputPixelBuffer(
                    from: pixelBuffer,
                    timestamp: Self.presentationTimestamp(from: sampleBuffer)
                )
            else {
                return
            }
            handler = VNImageRequestHandler(
                cvPixelBuffer: temporalPixelBuffer,
    ...
    private static func presentationTimestamp(from sampleBuffer: CMSampleBuffer) -> TimeInterval {
...
private final class TemporalGrayFrameBuffer {
    ...
    private var frames: [GrayFrame] = []

    init(maxFrameAge: TimeInterval) {
    ...
    func makeInputPixelBuffer(from sourcePixelBuffer: CVPixelBuffer, timestamp: TimeInterval) -> CVPixelBuffer? {
        guard let currentFrame = makeGrayFrame(from: sourcePixelBuffer, timestamp: timestamp) else {
            return nil
        }

        if let last = frames.last, last.width != currentFrame.width || last.height != currentFrame.height {
            frames.removeAll()
        }

        frames.append(currentFrame)
        frames = frames.filter { timestamp - $0.timestamp <= maxFrameAge }
        if frames.count > 3 {
            frames.removeFirst(frames.count - 3)
        }

        let current = currentFrame
        let previous = frame(offsetFromEnd: 1, fallback: current, timestamp: timestamp)
        let old = frame(offsetFromEnd: 2, fallback: current, timestamp: timestamp)
        return makeBGRAPixelBuffer(old: old, previous: previous, current: current)
    }

    private func frame(offsetFromEnd offset: Int, fallback: GrayFrame, timestamp: TimeInterval) -> GrayFrame {
        let index = frames.count - 1 - offset
        guard index >= 0 else {
            return fallback
        }
        let candidate = frames[index]
        guard timestamp - candidate.timestamp <= maxFrameAge else {
            return fallback
        }
        return candidate
    }

    private func makeGrayFrame(from pixelBuffer: CVPixelBuffer, timestamp: TimeInterval) -> GrayFrame? {
        ...
        for y in 0..<height {
            let sourceRow = source + y * bytesPerRow
            for x in 0..<width {
                let offset = x * 4
                let blue = Double(sourceRow[offset])
                let green = Double(sourceRow[offset + 1])
                let red = Double(sourceRow[offset + 2])
                pixels[y * width + x] = UInt8(min(max(0.114 * blue + 0.587 * green + 0.299 * red, 0), 255))
            }
        }

        return GrayFrame(width: width, height: height, timestamp: timestamp, pixels: pixels)
    }

    private func makeBGRAPixelBuffer(old: GrayFrame, previous: GrayFrame, current: GrayFrame) -> CVPixelBuffer? {
        ...
        for y in 0..<current.height {
            let destinationRow = destination + y * bytesPerRow
            for x in 0..<current.width {
                let sourceIndex = y * current.width + x
                let destinationOffset = x * 4
                destinationRow[destinationOffset] = current.pixels[sourceIndex]
                destinationRow[destinationOffset + 1] = previous.pixels[sourceIndex]
                destinationRow[destinationOffset + 2] = old.pixels[sourceIndex]
