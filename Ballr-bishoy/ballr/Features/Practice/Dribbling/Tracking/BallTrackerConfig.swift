...
struct BallTrackerConfig: Decodable {
    ...
    enum InputTemporalMode: String, Decodable {
        case singleRGB = "single_rgb"
        case temporalGray3 = "temporal_gray_3"
    }

    struct Input: Decodable {
        let shape: [Int]
        let temporalMode: InputTemporalMode

        enum CodingKeys: String, CodingKey {
            case shape
            case temporalMode = "temporal_mode"
        }

        init(from decoder: Decoder) throws {
            let container = try decoder.container(keyedBy: CodingKeys.self)
            shape = try container.decode([Int].self, forKey: .shape)
            temporalMode = try container.decodeIfPresent(InputTemporalMode.self, forKey: .temporalMode) ?? .singleRGB
        }
    }
    ...
    var inputTemporalMode: InputTemporalMode {
        input.temporalMode
    }
    ...
    var acceptedBallLabels: Set<String> {
        var labels = Set(["ball", "soccer-ball"])
        let configuredLabel = output.classLabels[String(ballrRuntimeDefaults.ballClassID)]
        if let configuredLabel, !configuredLabel.isEmpty {
            labels.insert(configuredLabel.lowercased())
        }
        return labels
    }
