...
struct FastTouchingJeffStandingOverlay: View {
    var body: some View {
...
struct FastTouchingJeffPlantedOverlay: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            let endX = max(-10, geometry.size.width - leadingInset - trailingInset - characterWidth + 10)
            let hiddenLegGap = characterWidth * (31.0 / 135.0)
...
struct FastTouchingJeffCountdownOverlay: View {
    let startedAt: Date
    var scoreText: String
    var timeText: String?
    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            ...
            GeometryReader { geometry in
                ...
                let endX = max(startX, geometry.size.width - leadingInset - trailingInset - characterWidth + 10)
                ...
                let hiddenLegGap = characterWidth * (31.0 / 135.0)
                let bodySettleDrop = hiddenLegGap * easeOut(jumpProgress)
                ...
                .offset(x: travelX + walkShake, y: -bodyLift + bodySettleDrop)
                .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .bottomLeading)
...
struct BallrOrangeJeffCharacterShape: View {
    ...
    var mouthScoreVisibility: CGFloat = 0
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            let mouthProgress = min(max(mouthOpenProgress, 0), 1)
            let expandedMouthWidth = 70.0 * scale
            let expandedMouthHeight = 34.0 * scale
            let expandedMouthLeft = 32.5 * scale
            let expandedMouthTop = roundedBodyTop + 28.0 * scale
            let animatedMouthWidth = mouthWidth + (expandedMouthWidth - mouthWidth) * mouthProgress
            let animatedMouthHeight = mouthHeight + (expandedMouthHeight - mouthHeight) * mouthProgress
            let animatedMouthLeft = mouthLeft + (expandedMouthLeft - mouthLeft) * mouthProgress
            let animatedMouthTop = mouthTop + (expandedMouthTop - mouthTop) * mouthProgress
            let animatedMouthCornerRadius = mouthCornerRadius + (17.0 * scale - mouthCornerRadius) * mouthProgress
            ...
            ZStack(alignment: .topLeading) {
                ...
                RoundedRectangle(cornerRadius: animatedMouthCornerRadius, style: .continuous)
                    .fill(legBlack)
                    .frame(width: animatedMouthWidth, height: animatedMouthHeight)
                    .offset(x: animatedMouthLeft, y: animatedMouthTop)
                    .opacity(faceVisibility)

                if let mouthScoreText {
                    Text(mouthScoreText)
                        .font(.ballr(size: 28 * scale, weight: .black))
                        .monospacedDigit()
                        .foregroundStyle(characterOrange)
                        .lineLimit(1)
                        .minimumScaleFactor(0.46)
                        .multilineTextAlignment(.center)
                        .frame(width: expandedMouthWidth, height: expandedMouthHeight)
                        .offset(x: expandedMouthLeft, y: expandedMouthTop - 1.0 * scale)
                        .opacity(mouthScoreVisibility * mouthProgress * faceVisibility)
                }
