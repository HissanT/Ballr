...
enum PianoTilesDifficulty: String, Identifiable {
    ...
    fileprivate var gameConfig: PianoTilesGameConfig {
        switch self {
        case .easy:
            return PianoTilesGameConfig(
                warningLeadDuration: 1.05,
                spawnGapScale: 2.05,
                tileSpeedScale: 0.90
            )
        case .medium:
...
fileprivate struct PianoTilesGameConfig {
    let warningLeadDuration: TimeInterval
    let spawnGapScale: Double
...
private struct PianoTilesGameState {
    ...
    private let config: PianoTilesGameConfig
    ...
    init(config: PianoTilesGameConfig) {
        self.config = config
    }
    ...
    mutating func reset() {
        ...
        nextSequenceIndex = 0
        nextSpawnElapsed = config.warningLeadDuration
    ...
    private mutating func spawnTilesIfNeeded(in size: CGSize, elapsed: TimeInterval) {
        ...
        if blockedSpawnNeedsReschedule {
            nextSpawnElapsed = elapsed + config.warningLeadDuration
            blockedSpawnNeedsReschedule = false
            return
        }
        ...
        let spawnGap = max(1.0, baseGap - lateGameTrim) * config.spawnGapScale
    ...
    private mutating func spawnTile(in size: CGSize) {
        ...
        let speed = baseSpeed * spec.speedMultiplier * config.tileSpeedScale
        tiles.append(
            PianoTile(
    ...
    func spawnWarning(elapsed: TimeInterval) -> PianoTileSpawnWarning? {
        ...
        let warningStart = nextSpawnElapsed - config.warningLeadDuration
        guard elapsed >= warningStart, elapsed < nextSpawnElapsed else {
            return nil
        }

        let progress = CGFloat((elapsed - warningStart) / config.warningLeadDuration)
