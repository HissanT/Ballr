...
private struct LevelTenGameState {
    ...
    private let spawnTopFraction: CGFloat = 0.55
    private let spawnBottomFraction: CGFloat = 0.75
    ...
    private func spawnBounds(in size: CGSize, radius: CGFloat) -> CGRect {
        ...
        let top = max(size.height * spawnTopFraction, radius + 76)
        let preferredBottom = min(size.height * spawnBottomFraction, size.height - radius - 58)
        let bottom = max(top, preferredBottom)
        return CGRect(
            x: horizontalPadding,
            y: top,
