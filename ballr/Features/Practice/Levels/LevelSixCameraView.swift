...
private final class LevelSixCoordinator: ObservableObject {
    ...
    func handle(frame: BallTrackerFrame, cameraController: BallTrackerCameraController) {
        ...
        if startPhase == .live, !hasEnded {
            ...
            case .miss:
                successfulHits = max(successfulHits - 1, 0)
                BallrDrillSoundPlayer.playIncorrect()
            case .bombHit:
                LevelSixSoundPlayer.playExplosion()
...
private final class LevelSixRenderView: UIView {
    ...
    private let goodProgressLayer = CAShapeLayer()
    ...
    private func configureLayers() {
        ...
        goodProgressLayer.fillColor = UIColor.clear.cgColor
        goodProgressLayer.strokeColor = UIColor.white.cgColor
        goodProgressLayer.lineWidth = 5
        goodProgressLayer.lineCap = .round
    ...
    private func renderGoodTarget(date: Date) {
        guard let goodTarget else {
            [goodFillLayer, goodOuterLayer, goodInnerLayer, goodProgressLayer].forEach {
                $0.isHidden = true
                $0.path = nil
            }
            return
        ...
        let progressRadius = max(goodTarget.radius - 16, 1)
        let remainingProgress = max(
            CGFloat(0.02),
            1.0 - CGFloat(targetAgeProgress(for: goodTarget, at: date))
        )

        goodFillLayer.isHidden = false
        goodOuterLayer.isHidden = false
        goodInnerLayer.isHidden = false
        goodProgressLayer.isHidden = false
        ...
        goodProgressLayer.path = UIBezierPath(
            arcCenter: goodTarget.center,
    ...
    private func targetAgeProgress(for target: LevelSixTarget, at date: Date) -> Double {
        min(max(date.timeIntervalSince(target.spawnedAt) / LevelSixGameState.targetLifetime, 0), 1)
    }
}
...
private struct LevelSixGameState {
    static let targetLifetime: TimeInterval = 3.0
    ...
    private let targetOuterXFractions: [CGFloat] = [0.08, 0.12, 0.16, 0.20]
    private let bombOuterXFractions: [CGFloat] = [0.04, 0.08, 0.12, 0.16]
    ...
    private let minimumTargetDistanceFromCenterMultiplier: CGFloat = 4.2
    private let minimumBombDistanceFromCenterMultiplier: CGFloat = 5.1
    ...
    private var lastTargetSlot: Int?
    private var lastBombSlot: Int?
    private var returnToCenterStartedAt: Date?
    private var completedCenterReturns = 0
    private var requiresCenterExitBeforeRespawn = false
    ...
    mutating func prepare(in size: CGSize, forceRespawn: Bool = false, allowSpawn: Bool = true) {
        ...
        if forceRespawn {
            ...
            requiresCenterExitBeforeRespawn = false
            spawnSidePair(in: size, timestamp: Date())
        }
    }
    ...
    mutating func step(
        ...
        if phase == .returningToCenter {
            ...
            let isBackAtCenter = isBallBackAtCenter(ballCenter: ballCenter, ballRadius: ballRadius, in: size)
            if !isBackAtCenter {
                requiresCenterExitBeforeRespawn = false
            }

            if isBackAtCenter, !requiresCenterExitBeforeRespawn {
                phase = .active
                showsCenterPrompt = false
                returnToCenterStartedAt = nil
                completedCenterReturns += 1
                spawnSidePair(in: size, timestamp: timestamp, ballCenter: ballCenter)
            }
            return nil
        }

        guard overlayState.isTracking, let ballDisplayRect else {
            if let goodTarget, targetAge(for: goodTarget, at: timestamp) >= Self.targetLifetime {
                return registerMiss(in: size, timestamp: timestamp)
            }
            return nil
        }
        ...
        if targetAge(for: goodTarget, at: timestamp) >= Self.targetLifetime {
            return registerMiss(in: size, timestamp: timestamp)
        }
    ...
    private mutating func spawnSidePair(in size: CGSize, timestamp: Date, ballCenter: CGPoint? = nil) {
        let radius = targetRadius(for: size)
        let center = centerPoint(in: size, radius: radius)
        let anchorCenter = ballCenter ?? center
        let targetSide = nextTargetSide()
        let bombSide = targetSide.opposite
        let targetSlot = nextSlot(
            fractions: targetOuterXFractions,
            side: targetSide,
            in: size,
            radius: radius,
            anchorCenter: anchorCenter,
            minimumDistance: radius * minimumTargetDistanceFromCenterMultiplier,
            avoiding: lastTargetSlot
        )
        let bombSlot = nextSlot(
            fractions: bombOuterXFractions,
            side: bombSide,
            in: size,
            radius: radius,
            anchorCenter: anchorCenter,
            minimumDistance: radius * minimumBombDistanceFromCenterMultiplier,
            avoiding: preferredBombAvoidedSlot(for: targetSlot)
        )
        goodTarget = LevelSixTarget(
            center: point(
                for: targetSide,
                fraction: targetOuterXFractions[targetSlot],
                in: size,
                radius: radius
            ),
            radius: radius,
        ...
        bombTarget = LevelSixTarget(
            center: point(
                for: bombSide,
                fraction: bombOuterXFractions[bombSlot],
                in: size,
                radius: radius
            ),
            radius: radius,
        ...
        lastBombSlot = bombSlot
    }

    private func targetAge(for target: LevelSixTarget, at timestamp: Date) -> TimeInterval {
        max(0, timestamp.timeIntervalSince(target.spawnedAt))
    }

    private mutating func registerMiss(in size: CGSize, timestamp: Date) -> LevelSixStepEvent {
        guard let goodTarget else {
            return .miss
        }
        ...
        returnToCenterStartedAt = timestamp
        showsCenterPrompt = true
        requiresCenterExitBeforeRespawn = true
        return .miss
    }
    ...
    private func point(for side: Side, fraction: CGFloat, in size: CGSize, radius: CGFloat) -> CGPoint {
        let xFraction = side == .left ? fraction : 1.0 - fraction
    ...
    private func nextSlot(
        ...
        avoiding avoidedSlot: Int?
    ) -> Int {
        guard fractions.count > 1 else {
            return 0
        }

        let validSlots = fractions.indices.filter { index in
            let candidate = point(for: side, fraction: fractions[index], in: size, radius: radius)
            return hypot(candidate.x - anchorCenter.x, candidate.y - anchorCenter.y) >= minimumDistance
        }

        var candidates = validSlots.isEmpty ? Array(fractions.indices) : validSlots
        if let avoidedSlot, candidates.count > 1 {
            candidates.removeAll { $0 == avoidedSlot }
        }
        return candidates.randomElement() ?? 0
    }

    private func preferredBombAvoidedSlot(for targetSlot: Int) -> Int? {
        if let lastBombSlot, lastBombSlot != targetSlot {
            return lastBombSlot
        }
        return targetSlot
    }
    ...
    private mutating func spawnPair(
        ...
        if let bestPair {
            goodTarget = LevelSixTarget(center: bestPair.goodCenter, radius: radius, spawnedAt: timestamp)
            bombTarget = LevelSixTarget(center: bestPair.bombCenter, radius: radius, spawnedAt: timestamp)
        } else {
            goodTarget = nil
            bombTarget = nil
        }
...
private enum LevelSixSoundPlayer {
    ...
    private static var explosionPlayer: AVAudioPlayer?

    static func playScore() {
        play(
            resource: "target_scored_sound_effect",
            fileExtension: "wav",
            player: &scorePlayer,
            errorLabel: "Level 6 score sound"
        )
    }

    static func playExplosion() {
        play(
            resource: "soundreality-explosion-fx-343683",
            fileExtension: "mp3",
            player: &explosionPlayer,
            errorLabel: "Level 6 explosion sound"
        )
    }
