...
struct PassingGateCameraView: View {
    @Environment(\.dismiss) private var dismiss
    @State private var selectedBallSpec: PassingGateBallSpec?

    var body: some View {
        Group {
            if let selectedBallSpec {
                PassingGateLiveCameraView(ballSpec: selectedBallSpec)
            } else {
                PassingGateSetupView(
...
private struct PassingGateSetupView: View {
    let onSelect: (PassingGateBallSpec) -> Void
    let onCancel: () -> Void

    var body: some View {
        ZStack {
            PassingGateSetupBackground()
                .ignoresSafeArea()
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                VStack(spacing: 12) {
                    ForEach(PassingGateBallSpec.presets) { spec in
                        Button {
                            onSelect(spec)
                        } label: {
                            HStack(spacing: 16) {
...
private struct PassingGateSetupBackground: View {
    var body: some View {
        ZStack {
...
private struct PassingGateLiveCameraView: View {
    @Environment(\.dismiss) private var dismiss
    @StateObject private var cameraController = BallTrackerCameraController()
    @StateObject private var coordinator = PassingGateCoordinator()
    @State private var showsQuitConfirmation = false

    let ballSpec: PassingGateBallSpec
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                PassingGateRenderSurface(coordinator: coordinator)
                    .ignoresSafeArea()
    ...
    private var topBar: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            HStack(spacing: 8) {
                PassingGateHudChip(title: "SCORE", value: "\(coordinator.score)", tint: .yellow)
                PassingGateHudChip(title: "BALL", value: coordinator.ballLabel, tint: .orange)
                PassingGateHudChip(title: "DEPTH", value: coordinator.depthText, tint: .white)
            }
        }
    }
...
private struct PassingGateBallSpec: Identifiable, Equatable {
    let id: String
    let label: String
    let sizeName: String
    ...
    static let presets = [
        PassingGateBallSpec(id: "3", label: "Size 3", sizeName: "3", diameterCM: 19.1),
        PassingGateBallSpec(id: "4", label: "Size 4", sizeName: "4", diameterCM: 20.3),
        PassingGateBallSpec(id: "5", label: "Size 5", sizeName: "5", diameterCM: 22.0)
    ]
}
...
private enum PassingGateThrowPhase {
    case idle
...
private struct PassingGateDepthEstimate {
    let distanceM: CGFloat
    let pixelDiameter: CGFloat
}

private struct PassingGateDepthSample {
    let timestamp: Date
...
private struct PassingGateWallCalibration {
    let wallDistanceM: CGFloat
    let impactPixelDiameter: CGFloat
    let confidence: CGFloat
}

private struct PassingGateImpact {
    let centerPixels: CGPoint
    let displayPoint: CGPoint?
    let horizontalDistanceCM: CGFloat
    let dxCM: CGFloat
    let dyCM: CGFloat
    let score: Int
    let timestamp: Date
...
private struct PassingGateDisplayMetrics {
    let innerHalfWidth: CGFloat
    let outerHalfWidth: CGFloat
    let height: CGFloat
}

private struct PassingGateTrackerOverlay {
...
private struct PassingGateThrowState {
    var phase: PassingGateThrowPhase = .idle
    var nearStartDepthM: CGFloat?
    var outboundFrames = 0
    var missingFrames = 0
    var furthestSample: PassingGateDepthSample?
    var eligibleSamples: [PassingGateDepthSample] = []
}

private final class PassingGateCoordinator: ObservableObject {
    ...
    private var wallCalibration: PassingGateWallCalibration?
    private var currentDepth: PassingGateDepthEstimate?
    ...
    private weak var renderView: PassingGateRenderView?

    func attach(renderView: PassingGateRenderView) {
    ...
    func handle(frame: BallTrackerFrame, cameraController: BallTrackerCameraController) {
        ...
        currentDepth = sample.map {
            PassingGateDepthEstimate(distanceM: $0.estimatedDistanceM, pixelDiameter: $0.smoothedDiameter)
        }
        updateHUDDepth()
    ...
    private func buildDepthSample(frame: BallTrackerFrame, focalLength: CGFloat) -> PassingGateDepthSample? {
    ...
    private func stepReference(sample: PassingGateDepthSample?, timestamp: Date) {
    ...
    private func stepCalibration(sample: PassingGateDepthSample?, timestamp: Date) {
        guard let sample else {
        ...
        wallCalibration = PassingGateWallCalibration(
        ...
        statusText = "Passing gate starts in 3"
    ...
    private func stepCountdown(timestamp: Date) {
        ...
        if elapsed >= countdownDuration {
            phase = .live
            phaseTitle = "LIVE"
            statusText = "Pass through the gate."
            self.countdownStartedAt = nil
        }
    }

    private func stepLive(
        sample: PassingGateDepthSample?,
        ...
        guard let wallCalibration, bullseyeTrackerPoint != nil else {
            phase = .calibration
            phaseTitle = "WALL DEPTH"
            statusText = "Passing gate needs calibration"
            return
        }
                ...
                throwState.eligibleSamples.count >= outboundMinimumFrames
            {
                ...
                if throwState.missingFrames > cooldownMissingFrames {
                    ...
                    if holdRemaining > 0 {
                        ...
                        statusText = "Last pass locked: \(remainingText(holdRemaining))"
                    } else {
                        throwState = PassingGateThrowState()
                    }
                }
        ...
        case .idle:
                ...
                sample.rawDistanceM - nearStart >= 0.12
            {
                ...
                statusText = "Pass toward the wall"
            }
        case .outboundCandidate:
            appendEligibleSample(sample)
            ...
            if sample.rawDistanceM < nearStart + 0.05 {
                throwState = PassingGateThrowState(nearStartDepthM: sample.rawDistanceM)
                statusText = "Pass toward the wall"
                return
            }
                ...
                sample.rawDistanceM - nearStart >= wallTravel * 0.40
            {
                ...
                statusText = "Outbound pass confirmed."
            }
        case .outboundConfirmed:
            appendEligibleSample(sample)
            let previous = throwState.eligibleSamples.dropLast().last
    ...
    private func appendEligibleSample(_ sample: PassingGateDepthSample) {
    ...
    private func isLiveSampleConsistent(_ sample: PassingGateDepthSample) -> Bool {
        guard
    ...
    private func lockImpact(
        _ impactSample: PassingGateDepthSample,
        frame: BallTrackerFrame,
        cameraController: BallTrackerCameraController
    ) {
        ...
        let horizontalCM = abs(dxCM)
        let awardedScore = scoreFromHorizontalDistance(horizontalCM)
        ...
        lastImpact = PassingGateImpact(
            centerPixels: centerPixels,
            displayPoint: displayPoint,
            horizontalDistanceCM: horizontalCM,
            dxCM: dxCM,
            dyCM: dyCM,
            score: awardedScore,
        ...
        statusText = awardedScore > 0
            ? "Gate scored: +\(awardedScore)"
            : "Wide by \(String(format: "%.1f", horizontalCM)) cm"
    }
    ...
    private func robustImpactCenter(impactTimestamp: Date) -> CGPoint? {
        var samples: [PassingGateDepthSample] = []
    ...
    private func publishRender() {
        let gateMetrics = gateDisplayMetrics()
        renderView?.update(
    ...
    private func gateDisplayMetrics() -> PassingGateDisplayMetrics? {
        guard
            let wallCalibration,
        ...
        return PassingGateDisplayMetrics(
            innerHalfWidth: displayLength(cm: PassingGateScoring.innerHalfWidthCM, wallDistanceM: wallCalibration.wallDistanceM, from: bullseyeDisplayPoint),
            outerHalfWidth: displayLength(cm: PassingGateScoring.outerHalfWidthCM, wallDistanceM: wallCalibration.wallDistanceM, from: bullseyeDisplayPoint),
            height: displayLength(cm: PassingGateScoring.gateHeightCM, wallDistanceM: wallCalibration.wallDistanceM, from: bullseyeDisplayPoint)
        )
    }

    private func displayLength(cm: CGFloat, wallDistanceM: CGFloat, from displayPoint: CGPoint) -> CGFloat {
        let focalLength = effectiveFocalLength(frameWidth: 1920)
        let normalizedLength = (cm / 100.0) * focalLength / wallDistanceM / 1920.0
        let approximateDisplayX = displayPoint.x + normalizedLength * viewSize.width
        return abs(approximateDisplayX - displayPoint.x)
    }
    ...
    private func scoreFromHorizontalDistance(_ horizontalCM: CGFloat) -> Int {
        PassingGateScoring.score(forHorizontalDistanceCM: horizontalCM)
    }
...
private enum PassingGateScoring {
    static let innerHalfWidthCM: CGFloat = 17.6
    static let outerHalfWidthCM: CGFloat = 44.0
    static let gateHeightCM: CGFloat = 55.0

    static func score(forHorizontalDistanceCM horizontalCM: CGFloat) -> Int {
        if horizontalCM <= innerHalfWidthCM {
            return 5
        }
        if horizontalCM <= outerHalfWidthCM {
            return 2
        }
        return 0
    }
...
private struct PassingGateRenderSurface: UIViewRepresentable {
    let coordinator: PassingGateCoordinator

    func makeUIView(context: Context) -> PassingGateRenderView {
        let view = PassingGateRenderView()
        coordinator.attach(renderView: view)
        return view
    }

    func updateUIView(_ uiView: PassingGateRenderView, context: Context) {
    }
}

private final class PassingGateRenderView: UIView {
    ...
    private var gateMetrics: PassingGateDisplayMetrics?
    private var lastImpact: PassingGateImpact?
    private var trackerOverlay = PassingGateTrackerOverlay(
        displayRect: nil,
        rawDisplayRect: nil,
    ...
    func update(
        bullseye: CGPoint?,
        gateMetrics: PassingGateDisplayMetrics?,
        lastImpact: PassingGateImpact?,
        trackerOverlay: PassingGateTrackerOverlay,
        lastScore: Int,
        phase: PassingGatePhase,
        score: Int
    ) {
    ...
    private func configureLayers() {
        ...
        trackerTextLayer.contentsScale = max(traitCollection.displayScale, 1)
        ...
        scoreLayer.contentsScale = max(traitCollection.displayScale, 1)
    ...
    private func renderTarget() {
        ...
        let metrics = gateMetrics ?? PassingGateDisplayMetrics(innerHalfWidth: 58, outerHalfWidth: 132, height: 122)
        let bottomY = min(max(bullseye.y, metrics.height + 18), bounds.height - 18)
        let topY = max(18, bottomY - metrics.height)
        let innerColor = UIColor(red: 0.63, green: 1.00, blue: 0.74, alpha: 0.88)
        let outerColor = UIColor(red: 0.62, green: 0.88, blue: 1.00, alpha: 0.62)

        addGateFill(
            CGRect(x: bullseye.x - metrics.outerHalfWidth, y: topY, width: metrics.outerHalfWidth * 2, height: bottomY - topY),
            color: outerColor.withAlphaComponent(0.08)
        )
        addGateFill(
            CGRect(x: bullseye.x - metrics.innerHalfWidth, y: topY, width: metrics.innerHalfWidth * 2, height: bottomY - topY),
            color: innerColor.withAlphaComponent(0.12)
        )

        addGatePost(x: bullseye.x - metrics.outerHalfWidth, topY: topY, bottomY: bottomY, color: outerColor, lineWidth: 3)
        addGatePost(x: bullseye.x + metrics.outerHalfWidth, topY: topY, bottomY: bottomY, color: outerColor, lineWidth: 3)
        addGatePost(x: bullseye.x - metrics.innerHalfWidth, topY: topY, bottomY: bottomY, color: innerColor, lineWidth: 4)
        addGatePost(x: bullseye.x + metrics.innerHalfWidth, topY: topY, bottomY: bottomY, color: innerColor, lineWidth: 4)
        addGateBase(fromX: bullseye.x - metrics.outerHalfWidth, toX: bullseye.x + metrics.outerHalfWidth, y: bottomY)

        let centerRect = CGRect(x: bullseye.x - 5, y: bottomY - 5, width: 10, height: 10)
        centerLayer.path = UIBezierPath(ovalIn: centerRect).cgPath

        addGateLabel("+5", color: innerColor, frame: CGRect(x: bullseye.x - 28, y: topY + 10, width: 56, height: 24))
        addGateLabel("+2", color: outerColor, frame: CGRect(x: bullseye.x + metrics.innerHalfWidth + 8, y: topY + 10, width: 56, height: 24))
    }

    private func addGateFill(_ rect: CGRect, color: UIColor) {
    ...
    private func addGatePost(x: CGFloat, topY: CGFloat, bottomY: CGFloat, color: UIColor, lineWidth: CGFloat) {
        let glow = CAShapeLayer()
        let glowPath = UIBezierPath()
        glowPath.move(to: CGPoint(x: x, y: topY))
        glowPath.addLine(to: CGPoint(x: x, y: bottomY))
        glow.path = glowPath.cgPath
        glow.strokeColor = color.withAlphaComponent(0.22).cgColor
        glow.lineWidth = lineWidth + 8
        glow.lineCap = .round
        targetLayer.addSublayer(glow)

        let post = CAShapeLayer()
        post.path = glowPath.cgPath
        post.strokeColor = color.cgColor
        post.lineWidth = lineWidth
        post.lineCap = .round
        post.shadowColor = UIColor.black.cgColor
        post.shadowOpacity = 0.35
        post.shadowRadius = 5
        post.shadowOffset = .zero
        targetLayer.addSublayer(post)
    }

    private func addGateBase(fromX: CGFloat, toX: CGFloat, y: CGFloat) {
        let base = CAShapeLayer()
        let path = UIBezierPath()
        path.move(to: CGPoint(x: fromX, y: y))
        path.addLine(to: CGPoint(x: toX, y: y))
        base.path = path.cgPath
        base.strokeColor = UIColor.white.withAlphaComponent(0.35).cgColor
        base.lineWidth = 2
        base.lineDashPattern = [8, 6]
        targetLayer.addSublayer(base)
    }

    private func addGateLabel(_ text: String, color: UIColor, frame: CGRect) {
        ...
        label.backgroundColor = UIColor.black.withAlphaComponent(0.42).cgColor
        label.cornerRadius = 6
        label.string = text
        label.frame = frame
        label.shadowColor = color.cgColor
...
private struct PassingGateHudChip: View {
    let title: String
    let value: String
    let tint: Color
...
private struct PassingGateLoadingOverlay: View {
    var body: some View {
        VStack(spacing: 14) {
            ProgressView()
                .tint(.white)
            Text("Starting Passing Gates...")
...
private struct PassingGateErrorOverlay: View {
    let message: String
