...
from ball_tracker_pose import PoseFrame, _point_to_segment_distance
...
JUGGLE_APEX_TRAVEL_RADIUS_FRACTION = 0.25
...
JUGGLE_GROUND_SCORE_MARGIN = 0.12
JUGGLE_REARM_CLEARANCE_RADIUS_FRACTION = 0.65
    ...
    class JuggleEventSequenceModel(nn.Module):
        def __init__(
            ...
            self.head = nn.Sequential(
                ...
                nn.Dropout(dropout),
                nn.Linear(hidden_size, len(EVENT_CLASS_ORDER)),
            )
...
class TorchJuggleEventClassifier:
    ...
    def classify(
        ...
        scores = {
            event_class: float(probabilities[index])
            for index, event_class in enumerate(EVENT_CLASS_ORDER)
        }
        best_class = max(scores, key=scores.get)
        return best_class, {event_class.value: score for event_class, score in scores.items()}
...
def update_juggle_state(
    ...
    *,
    ...
    elif state.last_live_track_at is not None and timestamp - state.last_live_track_at > JUGGLE_LOSS_RESET_SECONDS:
...
def _classify_candidate(
    ...
    *,
    ...
    proof_scores = {
        ...
        "thigh": _thigh_proof_score(proof_samples),
...
def _thigh_proof_score(samples: tuple[JuggleFrameSample, ...]) -> float:
    best_score = 0.0
    for sample_index, sample in enumerate(samples):
        if sample.center is None or not sample.pose_frame.available:
            continue
        player_height = _player_height(sample.pose_frame)
        threshold = max(sample.radius * 3.0, player_height * 0.11)
        weight = _proof_sample_weight(sample_index, len(samples))
        for side in ("left", "right"):
            hip = sample.pose_frame.keypoints.get(f"{side}_hip")
            knee = sample.pose_frame.keypoints.get(f"{side}_knee")
            if hip is None or knee is None:
                continue
            distance = _point_to_segment_distance(sample.center, hip, knee)
            best_score = max(best_score, _distance_score(distance, threshold) * weight)
    return best_score
    ...
    *,
    ...
    for sample_index, sample in enumerate(samples):
        if sample.center is None or not sample.pose_frame.available:
            continue
        player_height = _player_height(sample.pose_frame)
        threshold = max(sample.radius * radius_factor, player_height * height_factor)
        weight = _proof_sample_weight(sample_index, len(samples))
        for point_name in point_names:
            point = sample.pose_frame.keypoints.get(point_name)
            if point is None:
                continue
            distance = float(np.linalg.norm(sample.center - point))
            if include_ball_bottom:
                ball_bottom = np.array((sample.center[0], sample.center[1] + sample.radius), dtype=np.float32)
                distance = min(distance, float(np.linalg.norm(ball_bottom - point)))
            best_score = max(best_score, _distance_score(distance, threshold) * weight)
    return best_score
